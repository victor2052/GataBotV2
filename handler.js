        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (m.chat in global.DATABASE._data.chats || m.sender in global.DATABASE._data.users) {
            ...
            if (!['unbanuser.js', 'inv.js', 'link.js', 'creator.js', 'profile.js'].includes(name) && user && user.banned && !isROwner) {
              if (!opts['msgifbanned']) m.reply(`❰ ⚠️ ❱ *ESTAS BANEADO NO ESTES CHINGANDO* ❰ ⚠️ ❱ ${user.bannedReason ? `\n*Motivo:* *${user.bannedReason}*` : ''}

*👉 Puedes contactar a el propietario del Bot si crees que se trata de un error o para tratar el motivo de tú desbaneo*
